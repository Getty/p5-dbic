...
use DBIC::Carp qw/^SQL::Translator|^DBIC|^Try::Tiny/;
use DBIC::Exception;
...
# We're working with DBIC Schemas, not data streams.
# -------------------------------------------------------------------
sub parse {
    my ($tr, $data)   = @_;
    ...
    for (qw(DBIC::Schema DBIx::Schema package)) {
    ...
    DBIC::Exception->throw('No DBIC::Schema') unless ($dbicschema);

    if (!ref $dbicschema) {
      eval "require $dbicschema"
        or DBIC::Exception->throw("Can't load $dbicschema: $@");
    }
    ...
    for my $moniker (@monikers){
      my $source = $dbicschema->source($moniker);
       if ( $source->isa('DBIC::ResultSource::Table') ) {
         $table_monikers{$moniker}++;
      } elsif( $source->isa('DBIC::ResultSource::View') ){
...
"Arbitrary SQL through a custom ResultSource" in DBIC::Manual::Cookbook
or http://search.cpan.org/dist/DBIC/lib/DBIC/Manual/Cookbook.pod
...
sub _resolve_deps {
    ...
    if ( blessed($question)
        && $question->isa('DBIC::ResultSource::View') )
    {
...
C<SQL::Translator::Parser::DBIC> reads a DBIC schema,
interrogates the columns, and stuffs it all in an $sqlt_schema object.

Its primary use is in deploying database layouts described as a set
of L<DBIC> classes, to a database. To do this, see
L<DBIC::Schema/deploy>.

This can also be achieved by having DBIC export the schema as a
...
L<DBIC::Schema/create_ddl_dir>.

=head1 PARSER OPTIONS

=head2 dbic_schema

The DBIC schema (either an instance or a class name) to be parsed.
...
L<SQL::Translator>, L<DBIC::Schema>
