...
use DBIC::Carp '^DBIC|^Class::C3::Componentised';
use namespace::clean;
...
sub inject_base {
  ...
  my $keep_checking = ! (
    $target->isa ('DBIC::UTF8Columns')
      ||
    $target->isa ('DBIC::ForceUTF8')
  );
  ...
  while ($keep_checking && @complist) {
    ...
    for (qw/DBIC::UTF8Columns DBIC::ForceUTF8/) {
      if ($comp->isa ($_) ) {
        $keep_checking = 0; # no use to check from this point on
        carp_once "Use of $_ is strongly discouraged. See documentation of DBIC::UTF8Columns for more info\n"
    ...
    if (! $keep_checking) {
      ...
      my $base_store_column = do { require DBIC::Row; DBIC::Row->can ('store_column') };
      ...
      carp "Incorrect loading order of $comp by $target will affect other components overriding 'store_column' ("
          . join (', ', @broken)
          .'). Refer to the documentation of DBIC::UTF8Columns for more info'
        if @broken;
    }
