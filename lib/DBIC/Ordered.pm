...
# keep in mind that you can not invoke L<DBIC::Row/update> like
...
L<DBIC::Row> methods only. If you use the
L<DBIC::ResultSet> versions of
L<update|DBIC::ResultSet/update> or
L<delete|DBIC::ResultSet/delete>, all logic present in this
module will be bypassed entirely (possibly resulting in a broken
order-tree). Instead always use the
L<update_all|DBIC::ResultSet/update_all> and
L<delete_all|DBIC::ResultSet/delete_all> methods, which will
invoke the corresponding L<row|DBIC::Row> method on every
...
and using transactions (see L<DBIC::Storage/txn_do>)
...
you will need to call L<DBIC::Row/discard_changes> to get an object
