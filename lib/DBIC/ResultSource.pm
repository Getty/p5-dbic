...
use base qw/DBIC::ResultSource::RowParser DBIC/;

use DBIC::ResultSet;
use DBIC::ResultSourceHandle;
...
sources, for example L<DBIC::ResultSource::Table>. Table is the
default result source type, so one is created for you when defining a
result class as described in the synopsis above.

More specifically, the L<DBIC::Core> base class pulls in the
L<DBIC::ResultSourceProxy::Table> component, which defines
the L<table|DBIC::ResultSourceProxy::Table/table> method.
When called, C<table> creates and stores an instance of
L<DBIC::ResultSource::Table>. Luckily, to use tables as result
sources, you don't need to remember any of this.

Result sources representing select queries, or views, can also be
created, see L<DBIC::ResultSource::View> for full details.
...
sub new {
  ...
  $new->{resultset_class} ||= 'DBIC::ResultSet';
...
L<Result|DBIC::Manual::ResultClass> objects. You can change the name of the accessor
...
keys are currently recognised/used by DBIC:
...
L<SQL::Translator::Producer::DBIC::File> producer, or the
L<DBIC::Schema::Loader> module.
...
schema, see L<DBIC::Schema/deploy>.
...
from your schema, see L<DBIC::Schema/deploy>.
...
L<DBIC::Row/copy>. It is also used by
L<DBIC::Schema/deploy>.
...
create tables from your schema, see L<DBIC::Schema/deploy>.
...
See the note on L<DBIC::Row/new> for more information about possible
...
L<data_type|DBIC::ResultSource/data_type> whose values you want to
automatically generate using C<NEWID()>, unless they are a primary key in which
case this will be done anyway.

=item extra

This is used by L<DBIC::Schema/deploy> and L<SQL::Translator>
...
See
L<DBIC::Manual::Intro/The Significance and Importance of Primary Keys>
for more info.
...
name as the C<key> attribute to L<DBIC::ResultSet/find>. Then
...
the schema via L<DBIC::Schema/create_ddl_dir> or
L<DBIC::Schema/deploy>.
...
the L<DBIC::Schema/sqlt_deploy_hook>.

See L<DBIC::Manual::Cookbook/Adding Indexes And Functions To
...
handle, override L<DBIC::Schema/deploy> in your Schema class
and call L<dbh_do|DBIC::Storage::DBI/dbh_do>.
...
and use your own result inflator. See L<DBIC::ResultSet/result_class>
for more details.

Please note that setting this to something like
L<DBIC::ResultClass::HashRefInflator> will make every result unblessed
and make life more difficult.  Inflators like those are better suited to
temporary usage via L<DBIC::ResultSet/result_class>.
...
L<DBIC::ResultSet>, and set it here. If called with no arguments,
...
=item Arguments: L<\%attrs|DBIC::ResultSet/ATTRIBUTES>

=item Return Value: L<\%attrs|DBIC::ResultSet/ATTRIBUTES>
...
L<DBIC::ResultSet> produced from this result source.
...
=item Arguments: L<$schema?|DBIC::Schema>

=item Return Value: L<$schema|DBIC::Schema>

=back

  my $schema = $source->schema();

Sets and/or returns the L<DBIC::Schema> object to which this
  ...
  if (@_ > 1) {
    ...
    $_[0]->{schema} || do {
      ...
      $err .= ' You need to use $schema->thaw() or manually set'
            . ' $DBIC::ResultSourceHandle::thaw_schema while thawing.'
        if $_[0]->{_detached_thaw};

      DBIC::Exception->throw($err);
    };
  }
}
...
=item Return Value: L<$storage|DBIC::Storage>

=back

  $source->storage->debug(1);

Returns the L<storage handle|DBIC::Storage> for the current schema.
...
L<DBIC::Relationship> describes a series of methods which
...
=item Return Value: L<@rel_names|DBIC::Relationship>
...
=item Arguments: L<$rel_name|DBIC::Relationship>

=item Return Value: L<\%rel_data|DBIC::Relationship::Base/add_relationship>

=back

Returns a hash of relationship information for the specified relationship
name. The keys/values are as specified for L<DBIC::Relationship::Base/add_relationship>.
...
### API Design? What's that...? (a backwards compatible shim, kill me now)
  ...
  for (0,1) {
    ...
    if ( defined blessed $res_args[$_] ) {
      ...
      if ($res_args[$_]->isa('DBIC::ResultSet')) {
...
# TEMP
  ...
  $self->throw_exception( "Argument 'self_result_object' must be an object inheriting from DBIC::Row" )
    if (
      exists $args->{self_result_object}
        and
      ( ! defined blessed $args->{self_result_object} or ! $args->{self_result_object}->isa('DBIC::Row') )
    )
  ;
...
# TEMP
    ...
    if (defined blessed $args->{foreign_values}) {
      ...
      $self->throw_exception( "Objects supplied as 'foreign_values' ($args->{foreign_values}) must inherit from DBIC::Row" )
        unless $args->{foreign_values}->isa('DBIC::Row');
...
=item Return Value: L<$source_handle|DBIC::ResultSourceHandle>

=back

Obtain a new L<result source handle instance|DBIC::ResultSourceHandle>
...
sub handle {
  return DBIC::ResultSourceHandle->new({
