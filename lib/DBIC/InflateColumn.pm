...
something else, see L<DBIC::FilterColumn>.

=head1 METHODS

=head2 inflate_column

Instruct L<DBIC> to inflate the given column.
...
sub inflate_column {
  ...
  $self->throw_exception("InflateColumn can not be used on a column with a declared FilterColumn filter")
    if defined $colinfo->{_filter_info} and $self->isa('DBIC::FilterColumn');
...
analogous to L<DBIC::Row/get_column> in that it only fetches a
...
analogous to L<DBIC::Row/set_column>.
...
as dirty. This is directly analogous to L<DBIC::Row/store_column>.
...
=item L<DBIC::Core> - This component is loaded as part of the
      C<core> L<DBIC> components; generally there is no need to
      load it directly
