...
derived from L<DBIC::ResultSource> objects.

Result objects are returned from L<DBIC::ResultSet>s using the
L<create|DBIC::ResultSet/create>, L<find|DBIC::ResultSet/find>,
L<next|DBIC::ResultSet/next> and L<all|DBIC::ResultSet/all> methods,
as well as invocations of 'single' (
L<belongs_to|DBIC::Relationship/belongs_to>,
L<has_one|DBIC::Relationship/has_one> or
L<might_have|DBIC::Relationship/might_have>)
relationship accessors of L<Result|DBIC::Manual::ResultClass> objects.

=head1 NOTE

All "Row objects" derived from a Schema-attached L<DBIC::ResultSet>
object (such as a typical C<< L<search|DBIC::ResultSet/search>->
L<next|DBIC::ResultSet/next> >> call) are actually Result
instances, based on your application's
L<Result Class|DBIC::Manual::Glossary/Result Class>.

L<DBIC::Row> implements most of the row-based communication with the
underlying storage, but a Result class B<should not inherit from it directly>.
Usually, Result classes inherit from L<DBIC::Core>, which in turn
combines the methods from several classes, one of them being
L<DBIC::Row>.  Therefore, while many of the methods available to a
L<DBIC::Core>-derived Result class are described in the following
documentation, it does not detail all of the methods available to Result
objects.  Refer to L<DBIC::Manual::ResultClass> for more info.
...
L<DBIC::ResultSource> instance that is attached to a
L<DBIC::Schema> with a valid connection.
...
L<DBIC::Relationship::Base/set_from_related> for you. When
...
For a more involved explanation, see L<DBIC::ResultSet/create>.
...
=head2 $column_accessor
  ...
  # (inflated column via DBIC::InflateColumn::DateTime)
...
L<accessor|DBIC::ResultSource/accessor> name given during the
L<Result Class|DBIC::Manual::ResultClass> L<column definition
|DBIC::ResultSource/add_columns>. Like L</set_column>, this
...
entirely new row into the database, use L<DBIC::ResultSet/create>.

To fetch an uninserted result object, call
L<new_result|DBIC::ResultSet/new_result> on a resultset.

This will also insert any uninserted, related objects held inside this
one, see L<DBIC::ResultSet/create> for more details.
...
sub insert {
  ...
  for my $rel_name (keys %related_stuff) {
    ...
    if (! $self->{_rel_in_storage}{$rel_name}) {
      next unless (blessed $rel_obj && $rel_obj->isa('DBIC::Row'));
  ...
  foreach my $rel_name (keys %related_stuff) {
    ...
    if (@cands && blessed $cands[0] && $cands[0]->isa('DBIC::Row')
    ) {
      my $reverse = $rsrc->reverse_relationship_info($rel_name);
...
not. This is set to true when L<DBIC::ResultSet/find>,
L<DBIC::ResultSet/create> or L<DBIC::Row/insert>
are invoked.
...
L<significance of primary keys|DBIC::Manual::Intro/The Significance and Importance of Primary Keys>
for more details).
...
hashref of the relationship, see L<DBIC::Relationship>. Any
database-level cascade or restrict will take precedence over a
DBIC-based cascading delete, since DBIC B<deletes the
main row first> and only then attempts to delete any remaining related
rows.

If you delete an object within a txn_do() (see L<DBIC::Storage/txn_do>)
...
See also L<DBIC::ResultSet/delete>.
...
to L<has_column|DBIC::ResultSource/has_column>.

Returns a raw column value from the result object, if it has already
been fetched from the database or set by an accessor.

If an L<inflated value|DBIC::InflateColumn> has been set, it
will be deflated and returned.

Note that if you used the C<columns> or the C<select/as>
L<search attributes|DBIC::ResultSet/ATTRIBUTES> on the resultset from
...
See L<DBIC::InflateColumn> for how to setup inflation.
...
L<DBIC::Relationship/has_many> key, and create the related
...
See also L<DBIC::Relationship::Base/set_from_related>.
...
=item Return Value: L<$result|DBIC::Manual::ResultClass> copy
...
the L<column info attribute|DBIC::ResultSource/add_columns>
...
L<cascade_copy|DBIC::Relationship::Base> attribute. C<cascade_copy>
...
=item Arguments: L<$result_source|DBIC::ResultSource>, \%columndata, \%prefetcheddata

=item Return Value: L<$result|DBIC::Manual::ResultClass>

=back

All L<DBIC::ResultSet> methods that retrieve data from the
...
your Result class. See L<DBIC::ResultSource/result_class>.

Different types of results can also be created from a particular
L<DBIC::ResultSet>, see L<DBIC::ResultSet/result_class>.
  ...
  if ($prefetch) {
    for my $rel_name ( keys %$prefetch ) {
      ...
      if (
        @{ $prefetch->{$rel_name} || [] }
          and
        ref($prefetch->{$rel_name}) ne $DBIC::ResultSource::RowParser::Util::null_branch_class
      ) {
...
Accessor to the L<DBIC::ResultSource> this object was created from.
...
L<DBIC::ResultSource/add_columns>
...
entire
 L<primary key|DBIC::Manual::Intro/The Significance and Importance of Primary Keys>
...
Note: If you are using L<DBIC::Storage::DBI::Replicated> as your
storage, a default of
L<< C<< { force_pool => 'master' } >>
|DBIC::Storage::DBI::Replicated/SYNOPSIS >>  is automatically set for
you. Prior to C<< DBIC 0.08109 >> (before 2010) one would have been
...
See L<DBIC::Schema/throw_exception>.
...
Actually implemented in L<DBIC::PK>
