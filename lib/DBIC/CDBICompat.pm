package DBIC::CDBICompat;

use strict;
use warnings;
use base qw/DBIC::Core DBIC::DB/;

# Modules CDBICompat needs that DBIC does not.
my @Extra_Modules = qw(
    Class::Trigger
...
DBIC::CDBICompat - Class::DBI Compatibility layer.

=head1 SYNOPSIS

  package My::CDBI;
  use base qw/DBIC::CDBICompat/;

  ...continue as Class::DBI...

=head1 DESCRIPTION

DBIC features a fully featured compatibility layer with L<Class::DBI>
and some common plugins to ease transition for existing CDBI users.

This is not a wrapper or subclass of DBIC but rather a series of plugins.  The result being that even though you're using the Class::DBI emulation layer you are still getting DBIC objects.  You can use all DBIC features and methods via CDBICompat.  This allows you to take advantage of DBIC features without having to rewrite your CDBI code.


=head2 Plugins

CDBICompat is good enough that many CDBI plugins will work with CDBICompat, but many of the plugin features are better done with DBIC methods.
