...
BEGIN {
  use DBIC;
  die('The following modules are required for Replication ' . DBIC::Optional::Dependencies->req_missing_for ('replicated') . "\n" )
    unless DBIC::Optional::Dependencies->req_ok_for ('replicated');
}

use Moose;
use DBIC::Storage::DBI;
use DBIC::Storage::DBI::Replicated::Pool;
use DBIC::Storage::DBI::Replicated::Balancer;
use DBIC::Storage::DBI::Replicated::Types qw/BalancerClassNamePart DBICSchema DBICStorageDBI/;
...
L<DBIC::Storage::DBI::Replicated::Pool> and the replicants attribute for more.
...
Basically, any method request that L<DBIC::Storage::DBI> would normally
...
L<DBIC>. See L<DBIC::Optional::Dependencies> for more details.
...
The underlying L<DBIC::Schema> object this storage is attaching
...
to: L<DBIC::Storage::DBI::Replicated::Pool>.
...
has 'pool_type' => (
  is=>'rw',
  isa=>ClassName,
  default=>'DBIC::Storage::DBI::Replicated::Pool',
  handles=>{
    'create_pool' => 'new',
  },
);
...
See L<DBIC::Storage::DBI::Replicated::Pool> for available arguments.
...
has 'balancer_type' => (
  ...
  coerce=>1,
  required=>1,
  default=> 'DBIC::Storage::DBI::Replicated::Balancer::First',
  handles=>{
    'create_balancer' => 'new',
  },
);
...
Contains a hashref of initialized information to pass to the Balancer object.
See L<DBIC::Storage::DBI::Replicated::Balancer> for available arguments.
...
Is a L<DBIC::Storage::DBI::Replicated::Pool> or derived class.  This is a
...
has 'pool' => (
  is=>'ro',
  isa=>'DBIC::Storage::DBI::Replicated::Pool',
  lazy_build=>1,
  handles=>[qw/
    connect_replicants
    replicants
...
Is a L<DBIC::Storage::DBI::Replicated::Balancer> or derived class.  This
is a class that takes a pool (L<DBIC::Storage::DBI::Replicated::Pool>)

=cut

has 'balancer' => (
  is=>'rw',
  isa=>'DBIC::Storage::DBI::Replicated::Balancer',
  lazy_build=>1,
...
my $method_dispatch = {
  writer => [qw/
    ...
    _do_query
    _dbh_execute
  /, Class::MOP::Class->initialize('DBIC::Storage::DBIHacks')->get_method_list ],
  reader => [qw/
    select
    select_single
    columns_info_for
  ...
  /,(
    ...
    grep
      ...
      ( Class::MOP::Class->initialize('DBIC::Storage::DBI')->get_all_method_names )
  )],
};

if (DBIC::_ENV_::DBICTEST) {
  ...
  if (my @cant = grep { ! DBIC::Storage::DBI->can($_) } keys %$seen) {
    die(join "\n", '',
...
Defines an object that implements the read side of L<DBIC::Storage::DBI>.
...
Defines an object that implements the write side of L<DBIC::Storage::DBI>,
...
around connect_info => sub {
  ...
  return preserve_context {
    ...
    DBIC::Storage::DBI::Replicated::WithDSN->meta->apply($master);
...
L<DBIC::Schema> when instantiating its storage passed itself as the
...
sub _build_master {
  my $self = shift @_;
  my $master = DBIC::Storage::DBI->new($self->schema);
  $master
}
...
top of the args, since L<DBIC::Storage::DBI> needs it, and any
L<connect_info|DBIC::Storage::DBI/connect_info>
...
sets the L<DBIC::Storage::DBI/unsafe> option on all storages or returns
...
sets the L<DBIC::Storage::DBI/disable_sth_caching> option on all storages
...
returns the highest Replicant L<DBIC::Storage::DBI/lag_behind_master>
setting
...
L<DBIC::Storage::DBI/is_replicating>
...
calls L<DBIC::Storage::DBI/connect_call_datetime_setup> for all storages
...
calls L<DBIC::Storage::DBI/connect_call_rebase_sqlmaker> for all storages
...
L<< $result->discard_changes|DBIC::Row/discard_changes >>)
