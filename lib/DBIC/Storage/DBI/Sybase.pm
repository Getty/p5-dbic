...
sub _init {
  ...
  my $self = shift;
  if (! $self->isa('DBIC::Storage::DBI::Sybase::FreeTDS') and $self->_using_freetds) {
    require DBIC::Storage::DBI::Sybase::FreeTDS;
    ...
    no strict 'refs';
    @{"${trait_class}::ISA"} = ($class, 'DBIC::Storage::DBI::Sybase::FreeTDS', @isa);
