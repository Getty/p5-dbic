package DBIC::Storage::DBI::Cursor;

use strict;
use warnings;

use base 'DBIC::Cursor';

use Try::Tiny;
use Scalar::Util qw(refaddr weaken);
use DBIC::_Util 'detected_reinvoked_destructor';
use namespace::clean;
...
DBIC::Storage::DBI::Cursor - Object representing a query cursor on a
resultset.
...
A Cursor represents a query cursor on a L<DBIC::ResultSet> object. It
allows for traversing the result set with L</next>, retrieving all results with
L</all> and resetting the cursor with L</reset>.

Usually, you would use the cursor methods built into L<DBIC::ResultSet>
to traverse it. See L<DBIC::ResultSet/next>,
L<DBIC::ResultSet/reset> and L<DBIC::ResultSet/all> for more
information.
...
Returns a new L<DBIC::Storage::DBI::Cursor> object.
...
sub all {
  ...
  (
    DBIC::_ENV_::SHUFFLE_UNORDERED_RESULTSETS
      and
    ! $self->{attrs}{order_by}
      and
    require List::Util
...
sub sth {
  ...
  if (@_) {
    ...
    $self->{sth} = $_[0];
    $self->{_pid} = $$ if ! DBIC::_ENV_::BROKEN_FORK and $_[0];
  }
  elsif ($self->{sth} and ! $self->{_done}) {

    my $invalidate_handle_reason;

    if (DBIC::_ENV_::HAS_ITHREADS and $self->{_intra_thread} ) {
      $invalidate_handle_reason = 'Multi-thread';
    }
    elsif (!DBIC::_ENV_::BROKEN_FORK and $self->{_pid} != $$ ) {
...
Resets the cursor to the beginning of the L<DBIC::ResultSet>.
