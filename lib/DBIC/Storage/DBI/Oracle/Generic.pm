...
__PACKAGE__->sql_maker_class('DBIC::SQLMaker::Oracle');
...
L<DBIC::Storage::DBI::Oracle::WhereJoins> is for C<(+)> joins in Oracle
versions before 9.0.
...
sub _dbh_execute {
  ...
  return DBIC::Storage::BlockRunner->new(
    storage => $self,
    wrap_txn => 0,
    retry_handler => sub {
...
In L<connect_info|DBIC::Storage::DBI/connect_info> to set the session nls
date, and timestamp values for use with L<DBIC::InflateColumn::DateTime>
...
To get more than second precision with L<DBIC::InflateColumn::DateTime>
...
### https://github.com/Perl5/DBIC/commit/5db2758de6
#
...
the L<DBIC::Relationship> name is shortened and appended with half of an
MD5 hash.

See L<DBIC::Storage::DBI/relname_to_table_alias>.
...
It uses the same syntax as L<DBIC::ResultSet/search>
...
It uses the same syntax as L<DBIC::ResultSet/order_by>
