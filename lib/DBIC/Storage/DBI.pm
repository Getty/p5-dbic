...
use base qw/DBIC::Storage::DBIHacks DBIC::Storage/;
use mro 'c3';
...
use DBIC::_Util qw(quote_sub perlstring serialize detected_reinvoked_destructor sigwarn_silencer);
use namespace::clean;

# default cursor class, overridable in connect_info attributes
__PACKAGE__->cursor_class('DBIC::Storage::DBI::Cursor');
...
__PACKAGE__->sql_maker_class('DBIC::SQLMaker');
...
for my $meth (keys %$storage_accessor_idx, qw(
  ...
  quote_sub
    ...
    if (
      ...
      goto $_[0]->can(%2$s) unless DBIC::_ENV_::BROKEN_GOTO;
...
L<DBIC::Storage> for general information.  This pod only
...
sub DESTROY {
  ...
  $_[0]->_verify_pid unless DBIC::_ENV_::BROKEN_FORK;
...
This method is normally called by L<DBIC::Schema/connection>, which
...
set C<AutoCommit> to either I<0> or I<1>.  L<DBIC> further
recommends that it be set to I<1>, and that you perform transactions
via our L<DBIC::Schema/txn_do> method.  L<DBIC> will set it
...
of available limit dialects see L<DBIC::SQLMaker::LimitDialects>.
...
If this option is true, L<DBIC> will use savepoints when nesting
...
sub connect_info {
  ...
  if (keys %attrs and ref $args[0] ne 'CODE') {
    carp_unique (
      ...
      . 'DBIC::Storage::DBI for more info). If you still want to '
...
sub _populate_dbh {
  ...
  $_[0]->_conn_pid($$) unless DBIC::_ENV_::BROKEN_FORK; # on win32 these are in fact threads
  ...
  if ((not $self->_driver_determined) && (not $self->{_in_determine_driver})) {
    ...
    if (ref($self) eq __PACKAGE__) {
      ...
      if ($driver) {
        ...
        my $storage_class = "DBIC::Storage::DBI::${mapped_driver}";
...
sub _determine_connector_driver {
  ...
  my $subclass = "DBIC::Storage::DBI::${conn}::${dbtype}";
...
L<connection option|/DBIC specific connection attributes>
...
mix and match old and new within the same codebase as follows:
  ...
  package DBIC::Awesomer::SQLMaker;
  ...
  my $experimental_schema = $original_schema->connect(
    ...
    },
    {
      ...
      # https://metacpan.org/pod/DBIC::Storage::DBI#on_connect_call
      on_connect_call => [ [ rebase_sqlmaker => 'DBIC::Awesomer::SQLMaker' ] ],
    },
  );
  ...
  my $dbh_error_handler_installer = sub {
    ...
    $_[1]->{HandleError} = bless sub {
      if ($weak_self) {
        ...
        # the scope of DBIC
        DBIC::Exception->throw("DBI Exception (unhandled by DBIC, ::Schema GCed): $_[0]");
      }
      ...
      unless ($dbh->{RaiseError}) {
        carp( ref $info->[0] eq 'CODE'
          ...
          ? "The 'RaiseError' of the externally supplied DBI handle is set to false. "
           ."DBIC will toggle it back to true, unless the 'unsafe' connect "
...
sub _gen_sql_bind {
  ...
  if (
    ...
    carp_unique 'DateTime objects passed to search() are not supported '
      ...
      . 'DBIC::Manual::FAQ. To disable this warning for good '
...
sub insert {
  ...
  if (my $retlist = $sqla_opts->{returning}) {  # if IR is supported - we will get everything in one set
    ...
    if (%retrieve_cols) {
      ...
      my $cur = DBIC::ResultSet->new($source, {
...
to L<DBIC::Schema/connect>. For a list of available limit dialects
see L<DBIC::SQLMaker::LimitDialects>.
...
currently only used by L<::AutoCast|DBIC::Storage::DBI::AutoCast> and
L<::Sybase::ASE|DBIC::Storage::DBI::Sybase::ASE>.
...
L<DBIC::Row/set_column> decides whether to mark the column as
...
sub create_ddl_dir {
  ...
  unless (DBIC::Optional::Dependencies->req_ok_for ('deploy')) {
    $self->throw_exception("Can't create a ddl file without " . DBIC::Optional::Dependencies->req_missing_for ('deploy') );
  }
...
Returns the statements used by L<DBIC::Storage/deploy>
and L<DBIC::Schema/deploy>.
...
from L<DBIC::Schema/ddl_filename>, the schema name and the C<$version>.
...
sub deployment_statements {
  ...
  unless (DBIC::Optional::Dependencies->req_ok_for ('deploy') ) {
    $self->throw_exception("Can't deploy without a ddl_dir or " . DBIC::Optional::Dependencies->req_missing_for ('deploy') );
  }
...
L<DBIC> uses L<DBIC::Relationship> names as table aliases in
queries.

This hook is to allow specific L<DBIC::Storage> drivers to change the
...
(the default) combined with L<txn_do|DBIC::Storage/txn_do> for
transaction support.
