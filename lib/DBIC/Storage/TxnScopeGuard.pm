package DBIC::Storage::TxnScopeGuard;
...
use DBIC;
use DBIC::_Util qw(is_exception detected_reinvoked_destructor);
use DBIC::Carp;
use namespace::clean;
...
sub DESTROY {
  ...
  $self->{storage}->_verify_pid unless DBIC::_ENV_::BROKEN_FORK;
...

  {
    ...
    carp 'A DBIC::Storage::TxnScopeGuard went out of scope without explicit commit or error. Rolling back.'
    ...
    if ( $rollback_exception and (
      ! defined blessed $rollback_exception
          or
      ! $rollback_exception->isa('DBIC::Storage::NESTED_ROLLBACK_EXCEPTION')
    ) ) {
      # append our text - THIS IS A TEMPORARY FIXUP!
      # a real stackable exception object is in the works
      if (ref $exception eq 'DBIC::Exception') {
...
DBIC::Storage::TxnScopeGuard - Scope-based transaction handling
...
right thing with transactions in DBIC.
...
implicitly calling L<DBIC::Storage/txn_begin>. Expects a
L<DBIC::Storage> object as its only argument.
...
the transaction is rolled back, via L<DBIC::Storage/txn_rollback>

=cut

=head1 SEE ALSO

L<DBIC::Schema/txn_scope_guard>.
