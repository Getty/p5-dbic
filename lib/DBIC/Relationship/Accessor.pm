...
our %_pod_inherit_config =
  (
   class_map => { 'DBIC::Relationship::Accessor' => 'DBIC::Relationship' }
  );
      ...
      if (@_) {
        ...
        return undef if (
          $relcond->{join_free_condition}
            and
          $relcond->{join_free_condition} ne DBIC::_Util::UNRESOLVABLE_CONDITION
            and
...
EOC
    ...
    quote_sub "${class}::${rel}", sprintf( <<'EOC', perlstring $rel );
      DBIC::_ENV_::ASSERT_NO_INTERNAL_WANTARRAY and my $sog = DBIC::_Util::fail_on_internal_wantarray;
