...
methods, for predefined ones, look in L<DBIC::Relationship>.
...
L<DBIC::ResultSet/search>. The return value will be passed directly to
L<DBIC::SQLMaker> and the resulting SQL will be used verbatim as the
...
into hashref form by calling L<get_columns|DBIC::Row/get_columns>.
...
L<< $result->create_related|DBIC::Relationship::Base/create_related >>),
...
The L<standard ResultSet attributes|DBIC::ResultSet/ATTRIBUTES> may
...
By default, DBIC cascades deletes across C<has_many>,
...
before DBIC gets to perform the cascaded operation.

=item cascade_update

By default, DBIC cascades updates across C<has_one> and
...
specified globally in the args to L<DBIC::Schema/deploy> or
L<DBIC::Schema/create_ddl_dir>. Default is on, set to 0 to disable.
...
DBIC::ResultSourceProxy to set up Accessors and Proxies.
...
=item Return Value: L<$related_resultset|DBIC::ResultSet>

=back

  $rs = $cd->related_resultset('artist');

Returns a L<DBIC::ResultSet> for the relationship named
$rel_name.
...
=item Return Value: L<$result|DBIC::Manual::ResultClass> | L<$related_resultset|DBIC::ResultSet> | undef
...
This will return either a L<Result|DBIC::Manual::ResultClass> or a
L<ResultSet|DBIC::ResultSet>, depending on if the relationship is
...
=item Arguments: $rel_name, $cond?, L<\%attrs?|DBIC::ResultSet/ATTRIBUTES>

=item Return Value: L<$resultset|DBIC::ResultSet> (scalar context) | L<@result_objs|DBIC::Manual::ResultClass> (list context)

=back

Run a search on a related resultset. The search will be restricted to the
results represented by the L<DBIC::ResultSet> it was called
upon.

See L<DBIC::ResultSet/search_related> for more information.
...
your storage until you call L<DBIC::Row/insert> on it.
...
and C<new> in L<DBIC::ResultSet> for details.
...
=item Arguments: $rel_name, \%col_data | @pk_values, { key => $unique_constraint, L<%attrs|DBIC::ResultSet/ATTRIBUTES> }?
...
See L<DBIC::ResultSet/find> for details.
...
=item Arguments: $rel_name, \%col_data, { key => $unique_constraint, L<%attrs|DBIC::ResultSet/ATTRIBUTES> }?

=item Return Value: L<$result|DBIC::Manual::ResultClass>

=back

Find a result object of a related class.  See L<DBIC::ResultSet/find_or_new>
for details.
...
L<DBIC::ResultSet/find_or_create> for details.
...
L<DBIC::ResultSet/update_or_create> for details.
...
=item Arguments: $rel_name, L<$result|DBIC::Manual::ResultClass>
...
L<DBIC::ResultSet/create>, or pass an object to a belongs_to accessor.

The columns are only set in the local copy of the object, call
L<update|DBIC::Row/update> to update them in the storage.
...
=item Arguments: (\%col_data | L<$result|DBIC::Manual::ResultClass>), \%link_col_data?
...
See L<DBIC::Relationship/many_to_many> for additional details.
...
=item Arguments: (\@hashrefs_of_col_data | L<\@result_objs|DBIC::Manual::ResultClass>), $link_vals?
...
=item Arguments: L<$result|DBIC::Manual::ResultClass>
