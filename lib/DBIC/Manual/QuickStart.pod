# PODNAME: DBIC::Manual::QuickStart
# ABSTRACT: up and running with DBIC in 10 minutes
...
First, install DBIC like you do with any other CPAN distribution.
...
mentioned earlier, the next command will download and unpack it:
    ...
    $ perl -mCPAN -e'CPAN::Shell->look("DBIC")'
    DBIC$ cd examples/Schema

Inspect the database:

    DBIC/examples/Schema$ sqlite3 db/example.db .dump
...
to the tables in the database. L<DBIC::Manual::Example> shows how to
write all that Perl code. That is almost never necessary, though. Instead use
L<dbicdump> (part of the distribution L<DBIC::Schema::Loader>) to
...
A L<schema|DBIC::Manual::Glossary/Schema> object represents the database.
...
L<resultset|DBIC::Manual::Glossary/ResultSet> object.
...
L<Result|DBIC::Manual::Glossary/Result> objects represent a row and
...
=head2 Resetting the database
    ...
    DBIC/examples/Schema$ rm -f db/example.db

    # create database and set up tables from definition
    DBIC/examples/Schema$ sqlite3 db/example.db < db/example.sql

    # fill them with data
    DBIC/examples/Schema$ perl ./insertdb.pl

    # delete the schema classes files
    DBIC/examples/Schema$ rm -rf MyApp

    # recreate schema classes files from database file
    DBIC/examples/Schema$ dbicdump \
...
L<DBIC::Manual::SQLHackers>.

Continue with L<DBIC::Tutorial> and
L<DBIC/"WHERE TO START READING">.
