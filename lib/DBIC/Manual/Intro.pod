# PODNAME: DBIC::Manual::Intro
# ABSTRACT: Introduction to DBIC

=head1 INTRODUCTION

You're bored with SQL, and want a native Perl interface for your database?
...
DBIC needs to know what your Table structure looks like.  You
do that by defining Result classes. Result classes are defined by
calling methods proxied to L<DBIC::ResultSource>.  Each Result
...
be creating a L<DBIC::ResultSet> from its ResultSource.  For example, the
results of:
...
DBIC doesn't limit you to creating only simple ResultSets -- if you
...
The important thing to understand:
  ...
  creating a DBIC::ResultSet.

=head2 Search is like "prepare"

DBIC tends to wait until it absolutely must fetch information from the
...
L<Result|DBIC::Manual::ResultClass> objects.

=head1 SETTING UP DBIC

Let's look at how you can set and use your first native L<DBIC> tree.
...
DBIC::Schema::Loader>, which shows you how to use
L<DBIC::Schema::Loader>.
...
defaulted to be DBIC::ResultSet objects). You can change the
result and resultset namespaces by using options to the
L<DBIC::Schema/load_namespaces> call.
...
DBIC doesn't directly use most of this data yet, but various related
...
See L<DBIC::Schema/deploy> for details.

See L<DBIC::ResultSource> for more details of the possible column
attributes.
...
See L<DBIC::Relationship> for more information about the various types of
available relationships and how you can design your own.

=head2 Using DBIC::Schema::Loader

This module (L<DBIC::Schema::Loader>) is an external module, and not part
of the L<DBIC> distribution. It inspects your database, and automatically
creates classes for all the tables in your schema.

The simplest way to use it is via the L<dbicdump> script from the
L<DBIC::Schema::Loader> distribution. For example:
...
See the L<DBIC::Schema::Loader> and L<DBIC::Schema::Loader::Base>
...
Note that L<DBIC::Schema> does not cache connections for you. If you use
...
See L<DBIC::Storage::DBI/connect_info> for more information about
...
L<DBIC::Schema::Loader>, you can start interacting with your database.

To access your database using your $schema object, you can fetch a
L<DBIC::Manual::Glossary/"ResultSet"> representing each of your tables by
...
Call C<update> to save your changes to the database (by executing the actual
C<UPDATE> statement):
...
L<DBIC> provides a few different ways to retrieve data from your
database.  Here's one example:

  # Find all of Santana's albums
  my $rs = $schema->resultset('Album')->search({ artist => 'Santana' });

In scalar context, as above, C<search> returns a L<DBIC::ResultSet>
...
For more examples of complex queries, see L<DBIC::Manual::Cookbook>.
...
For more information on what you can do with a L<DBIC::ResultSet>, see
L<DBIC::ResultSet/METHODS>.
...
The concept of a L<primary key|DBIC::ResultSource/set_primary_key> in
DBIC warrants special discussion. The formal definition (which somewhat
...
L<delete|DBIC::Row/delete>,
L<update|DBIC::Row/update>,
L<discard_changes|DBIC::Row/discard_changes>,
etc.) DBIC will use the values of the
L<primary key|DBIC::ResultSource/set_primary_key> columns to populate
the C<WHERE> clause necessary to accomplish the operation. This is why it is
important to declare a L<primary key|DBIC::ResultSource/set_primary_key>
...
Note that DBIC is smart enough to store a copy of the PK values before
any row-object changes take place, so even if you change the values of PK
columns the C<WHERE> clause will remain correct.

If you elect not to declare a C<primary key>, DBIC will behave correctly
...
L<DBIC::ResultSet> CRUD methods:
L<search|DBIC::ResultSet/search>,
L<update|DBIC::ResultSet/update>,
L<delete|DBIC::ResultSet/delete>
...
information on this can be found in L<DBIC::Manual::Troubleshooting>

=head1 SEE ALSO

=over 4

=item * L<DBIC::Manual::Cookbook>
