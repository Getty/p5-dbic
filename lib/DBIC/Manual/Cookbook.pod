...
DBIC::Manual::Cookbook - Miscellaneous recipes

=head1 SEARCHING

=head2 Paged results

When you expect a large number of results, you can ask L<DBIC> for a
...
You can get a L<DBIC::ResultSet::Pager> object for the resultset
...
can be easily done with L<< $rs->first|DBIC::ResultSet/first
...
object.  L<< $rs->single|DBIC::ResultSet/single >> is designed
...
get the results as a L<DBIC::ResultSet>.

This is accomplished by defining a
L<ResultSource::View|DBIC::ResultSource::View> for your query,
...
... and you'll get back a perfect L<DBIC::ResultSet> (except, of course,
that you cannot modify the rows it contains, e.g. cannot call
L<update|DBIC::ResultSet/update> or
L<delete|DBIC::ResultSet/delete> on it).
...
L<DBIC::Schema/deploy>, add this line to your source definition, so that
...
L<DBIC::ResultSet/ATTRIBUTES>). You can control the C<AS> part of the
...
L<add_columns|DBIC::ResultSource/add_columns>), you just access it as
...
L<DBIC> supports C<GROUP BY> as follows:
...
Please see L<DBIC::ResultSet/ATTRIBUTES> documentation if you
...
If you're using L<DBIC::Schema/load_namespaces>, simply place the file
into the C<ResultSet> directory next to your C<Result> directory, and it will
be automatically loaded.

If however you are still using L<DBIC::Schema/load_classes>, first tell
DBIC to create an instance of the ResultSet class for you, in your
...
it can be accomplished with C<DBIC> when necessary by resorting to
literal SQL:
...
explained in L<DBIC::ResultSet/DBIC BIND VALUES>.
...
and L<GenericSubQ|DBIC::SQLMaker::LimitDialects/GenericSubQ> is either
too slow or does not work at all, you can try the
L<software_limit|DBIC::ResultSet/software_limit>
L<DBIC::ResultSet> attribute, which skips over records to simulate limits
in the Perl layer.
...
This requires that you have defined the L<DBIC::Relationship>. For example :
...
(see L<DBIC::Manual::Joining> for more details on aliasing ).
...
the artist name for any of the CD objects returned, L<DBIC> will go back
to the database:
...
Thankfully, L<DBIC> has a C<prefetch> attribute to solve this problem.
...
L<DBIC> has now prefetched all matching data from the C<artist> table,
...
B<Result-Subclass definition>
    ...
    # otherwise DBIC::ResultSourceProxy::Table will not be called
...
L<DBIC> classes are proxy classes, therefore some different
techniques need to be employed for more than basic subclassing.  In
this example we have a single user table that carries a boolean bit
for admin.  We would like to give the admin users
objects (L<DBIC::Row>) the same methods as a regular user but
...
L<DBIC::ResultSet> when inflating a result from storage.  So we
...
Alternatively you can use L<DBIC::DynamicSubclass> that implements
exactly the above functionality.

=head2 Skip result object creation for faster results

DBIC is not built for speed, it's built for convenience and
ease of use, but sometimes you just need to get the data, and skip the
fancy objects.

To do this simply use L<DBIC::ResultClass::HashRefInflator>.
...
L<DBIC::ResultSet/result_class> will replace any existing class
completely including any special components loaded using
load_components, eg L<DBIC::InflateColumn::DateTime>.

=head2 Get raw data for blindingly fast results

If the L<HashRefInflator|DBIC::ResultClass::HashRefInflator> solution
above is not fast enough for you, you can use a DBIC to return values
...
use the L<DBIC::ResultSet/select> attribute of L<DBIC::ResultSet/search> to force ordering).

=head1 RESULTSET OPERATIONS

=head2 Getting Schema from a ResultSet

To get the L<DBIC::Schema> object from a ResultSet, do the following:
...
ways, the obvious one is to use search:
  ...
  my $rs = $schema->resultset('Items')->search(
    ...
    {
       select => [ { sum => 'Cost' } ],
       as     => [ 'total_cost' ], # remember this 'as' is for DBIC::ResultSet not SQL
    }
  );
  my $tc = $rs->first->get_column('total_cost');

Or, you can use the L<DBIC::ResultSetColumn>, which gets
...
See L<DBIC::ResultSetColumn> for more documentation.
...
L<set_cache|DBIC::ResultSet/set_cache> method:
...
This is straightforward using L<ManyToMany|DBIC::Relationship/many_to_many>:
...
Mapping relationships across L<DB schemas|DBIC::Manual::Glossary/DB schema>
...
L<connection|DBIC::Schema/connection> method in your Schema class and
...
By overriding the L<connection|DBIC::Schema/connection>
...
L<connection|DBIC::Schema/connect>, as follows:
...
L<DBIC::Storage> and L<DBIC::Schema>.  Here is an
...
L<txn_do|DBIC::Storage/txn_do>: If the L<try|Try::Tiny/try>-block
around L<txn_do|DBIC::Storage/txn_do> fails, a rollback is issued.
...
=head2 Simple Transactions with DBIC::Storage::TxnScopeGuard

An easy way to use transactions is with
L<DBIC::Storage::TxnScopeGuard>. See L</Automatically creating
...
L<DBIC::Schema::Loader> will connect to a database and create a
L<DBIC::Schema> and associated sources by examining the database.

The recommend way of achieving this is to use the L<dbicdump> utility or the
L<Catalyst> helper, as described in
L<Manual::Intro|DBIC::Manual::Intro/Using DBIC::Schema::Loader>.

Alternatively, use the
L<make_schema_at|DBIC::Schema::Loader/make_schema_at> method:
...
In case you're going to use this "trick" together with L<DBIC::Schema/deploy> or
L<DBIC::Schema/create_ddl_dir> a table called "dual" will be created in your
...
Or use L<DBIC::ResultClass::HashRefInflator>
...
L<callback system|DBIC::ResultSource/sqlt_deploy_callback> if you wish
...
The following example shows simplistically how you might use DBIC to
...
Call L<DBIC::Schema/create_ddl_dir> as above under L</Creating DDL SQL>.
...
DBIC, export the modified schema and the conversion scripts as
in L</Creating DDL SQL>.

B<Deploy update to customers>

Add the L<DBIC::Schema::Versioned> schema component to your
...
L<DBIC::ResultSet/create> and L<DBIC::Row/update> family of
methods:
...
In conditions (e.g. C<\%cond> in the L<DBIC::ResultSet/search> family of
...
formatter returned by L<DBIC::Storage::DBI/datetime_parser> to format
any L<DateTime> objects you pass to L<search|DBIC::ResultSet/search>
conditions. Any L<Storage|DBIC::Storage> object attached to your
L<Schema|DBIC::Schema> provides a correct L<DateTime> formatter, so
...
L<search|DBIC::ResultSet/search> and L<DBIC::ResultSet/find>,
whereas L<create|DBIC::ResultSet/create> and
L<DBIC::Row/update> (but not L<DBIC::ResultSet/update>) are
L<DBIC::InflateColumn>-aware and will do the right thing when supplied
...
The L<DBIC::UTF8Columns> component handles storing selected
...
You want to start using the schema-based approach to L<DBIC>
(see L<DBIC::Manual::Intro/Setting it up manually>), but have an
...
want to move by hand. Try this nifty script instead:
  ...
  $translator->producer('SQL::Translator::Producer::DBIC::File');
...
L<DBIC> uses the L<Class::C3> package, which provides for redispatch of
...
L<Class::C3> with L<DBIC> can be found in
L<DBIC::Manual::Component>.
...
documentation. See also L<DBIC::Manual::Component> for more
ways to write your own base classes to do this.

People looking for ways to do "triggers" with DBIC are probably
...
When you enable L<DBIC::Storage>'s debugging it prints the SQL
executed as well as notifications of query completion and transaction
begin/commit.  If you'd like to profile the SQL you can subclass the
L<DBIC::Storage::Statistics> class and write your own profiling
mechanism:
...
You might want to check out L<DBIC::QueryLog> as well.
...
L<prefetch|DBIC::ResultSet/prefetch> relationships, where possible. See
L</Using joins and prefetch>.

=item *

Use L<populate|DBIC::ResultSet/populate> in void context to insert data
when you don't need the resulting L<result|DBIC::Manual::ResultClass> objects,
...
If using L<create|DBIC::ResultSet/create> instead, use a transaction and
commit every C<X> rows; where C<X> gives you the best performance without
locking the table for too long.

=item *

When selecting many rows, if you don't need full-blown L<DBIC::Row>
objects, consider using L<DBIC::ResultClass::HashRefInflator>.
...
L<DBIC|DBIC> programs can have a significant startup delay
...
If you are using
L<DBIC::Schema::Loader|DBIC::Schema::Loader> to build the
...
using L<DBIC::Schema::Loader> to dump the database schema once - see
L<make_schema_at|DBIC::Schema::Loader/make_schema_at> and
L<dump_directory|DBIC::Schema::Loader::Base/dump_directory> for more
details on creating static schemas from a database).

=head2 Move Common Startup into a Base Class

Typically L<DBIC> result classes start off with
...
If you are instead using the L<load_namespaces|DBIC::Schema/load_namespaces>
...
L<DBIC> normally caches all statements with
