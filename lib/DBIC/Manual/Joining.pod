# PODNAME: DBIC::Manual::Joining
# ABSTRACT: Manual on joining tables with DBIC

=head1 DESCRIPTION

This document should help you to use L<DBIC> if you are trying
to convert your normal SQL queries into DBIC based queries, if
...
then you should likely try the L<DBIC::Manual::Intro>
...
In L<DBIC> each relationship between two tables needs to first
be defined in the L<ResultSource|DBIC::Manual::Glossary/ResultSource> for the
...
comprehensively described in L<DBIC::Relationship>.
...
L<DBIC::Relationship> docs.

When performing either a L<search|DBIC::ResultSet/search> or a
L<find|DBIC::ResultSet/find> operation, you can specify which
C<relations> to also refine your results based on, using the
L<join|DBIC::ResultSet/join> attribute, like this:
...
read L<DBIC::ResultSet/search> and
L<DBIC::ResultSet/ATTRIBUTES>, but here's a quick break down:
...
output, see the L<DBIC::Manual::FAQ> for an explanation.
...
As an aside to all the discussion on joins, note that L<DBIC>
