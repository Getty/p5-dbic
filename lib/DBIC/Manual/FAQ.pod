...
DBIC::Manual::FAQ - Frequently Asked Questions (in theory)

=head1 DESCRIPTION

This document is intended as an anti-map of the documentation. If you
know what you want to do, but not how to do it in L<DBIC>, then
...
DBIC schema. If it's the former, look up the documentation for
...
Install L<DBIC::Schema::Loader> from CPAN, and read its documentation.

=item .. set up my DBIC classes manually?

Look at the L<DBIC::Manual::Example> and come back here if you get lost.

=item .. create my database tables from my DBIC schema?

Create your classes manually, as above. Write a script that calls
L<DBIC::Schema/deploy>. See there for details, or the
...
tables. See L<DBIC::Schema/connect> for details. The actual
...
you to use.  These are all listed in L<DBIC::Relationship>. If
...
be set up individually on each side. See L<DBIC::Relationship>
for details.
...
foreign key.  See L<DBIC::Relationship/belongs_to>.
...
as you like. See L<DBIC::Relationship::Base>.
...
relationship in the fullest sense.  For more info, read the documentation on L<DBIC::Relationship/many_to_many>.

=item .. stop DBIC from attempting to cascade deletes on my has_many and might_have relationships?

By default, DBIC cascades deletes and updates across
...
database?>. Find the
L<ResultSet|DBIC::Manual::Glossary/ResultSet> that you want to
search in, by calling C<< $schema->resultset('MySource') >> and call
C<search> on it. See L<DBIC::ResultSet/search>.
...
relationship between the two tables, see L<DBIC::Relationship>
for the join used by each relationship.

=item .. create joins with conditions other than column equality?

Currently, L<DBIC> can only create join conditions using
...
Behind the scenes, DBIC uses L<SQL::Abstract::Classic> to help construct
its SQL searches. So if you fail to find help in the
L<DBIC::Manual::Cookbook>, try looking in the L<SQL::Abstract::Classic>
documentation.
...
L<search|DBIC::ResultSet/search> and L<find|DBIC::ResultSet/find>
do not take L<DBIC::InflateColumn> into account, and so your L<DateTime>
object will not be correctly deflated into a format your RDBMS expects.

The L<datetime_parser|DBIC::Storage::DBI/datetime_parser> method on your
...
L<C<result_source>|DBIC::Row/result_source>.
...
whereas L<create|DBIC::ResultSet/create> and L<DBIC::Row/update>
(but not L<DBIC::ResultSet/update>) are
...
See the prefetch examples in the L<Cookbook|DBIC::Manual::Cookbook>.

=item .. fetch a whole column of data instead of a row?

Call C<get_column> on a L<DBIC::ResultSet>. This returns a
L<DBIC::ResultSetColumn>. See its documentation and the
L<Cookbook|DBIC::Manual::Cookbook> for details.
...
See the L<Cookbook|DBIC::Manual::Cookbook> for more details.

=item .. fetch a single (or topmost) row?

Use the L<DBIC::ResultSet/rows> and
L<DBIC::ResultSet/order_by> attributes to order your data and
pick off a single row.

See also L<DBIC::Manual::Cookbook/Retrieve one and only one row from a resultset>.

A less readable way is to ask a regular search to return 1 row, using
L<DBIC::ResultSet/slice>:
...
L<DBIC::Row/discard_changes> does just that by re-fetching the row from storage
...
|DBIC::ResultSet::Pager> with an API/behavior identical to that
...
L<creating|DBIC::ResultSet/create> a result object, the primary
...
primary key field from the sequence. To help DBIC find the next
...
The C<populate> method in L<DBIC::ResultSet> provides
efficient bulk inserts.

L<DBIC::Fixtures> provides an alternative way to do this.
...
You can use L<DBIC::InflateColumn> to accomplish YAML/JSON storage transparently.
...
When two tables ARE related, L<DBIC::Relationship::Base> provides many
...
what create_related() from L<DBIC::Relationship::Base> does, you could
add this to Book.pm:
...
And another method is to use L<Moose> with your L<DBIC> package.
	...
	extends 'DBIC::Core'; # Moose changes the way we define our parent (base) package
...
L<DBIC> runs the actual SQL statement as late as possible, thus
...
If your table lacks a primary key, DBIC can't work out which row
it should operate on, for example to delete or update.  However, a
UNIQUE constraint on one or more columns allows DBIC to uniquely
identify the row, so you can tell L<DBIC::ResultSource> these
...
Look at the tips in L<DBIC::Manual::Cookbook/"STARTUP SPEED">

=item How do I reduce the overhead of database queries?

You can reduce the overhead of object creation within L<DBIC>
using the tips in L<DBIC::Manual::Cookbook/"Skip result object creation for faster results">
and L<DBIC::Manual::Cookbook/"Get raw data for blindingly fast results">
...
See L<DBIC::Manual::Cookbook/Stringification>
