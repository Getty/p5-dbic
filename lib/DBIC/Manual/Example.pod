...
DBIC::Manual::Example - Simple CD database example

=head1 DESCRIPTION

This tutorial will guide you through the process of setting up and
testing a very basic CD database using SQLite, with DBIC::Schema
...
You'll need to install DBIC via CPAN, and you'll also need to
...
This example uses L<DBIC::Schema/load_namespaces> to load in the
appropriate L<Result|DBIC::Manual::ResultClass> classes from the
C<MyApp::Schema::Result> namespace, and any required
L<ResultSet|DBIC::ResultSet> classes from the
