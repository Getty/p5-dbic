package DBIC::InflateColumn::DateTime;
...
DBIC::InflateColumn::DateTime - Auto-create DateTime objects from date and datetime columns.
...
inflate/deflate with based on the type of DBIC::Storage::DBI::*
...
For more help with using components, see L<DBIC::Manual::Component/USING>.
...
Chains with the L<DBIC::Row/register_column> method, and sets
...
sub _pre_deflate_datetime {
  ...
  if (defined $info->{timezone}) {
    carp "You're using a floating timezone, please see the documentation of"
      . " DBIC::InflateColumn::DateTime for an explanation"
...
B<DEPRECATED> because this gets you into trouble using L<DBIC::Schema::Versioned>.
...
=item More information about the add_columns method, and column metadata,
      can be found in the documentation for L<DBIC::ResultSource>.
