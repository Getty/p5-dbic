...
sub scope_guard (&) {
  ...
  bless [ $_[0] ], 'DBIC::_Util::ScopeGuard';
}
{
  package #
    DBIC::_Util::ScopeGuard;

  sub DESTROY {
    &DBIC::_Util::detected_reinvoked_destructor;

    local $@ if DBIC::_ENV_::UNSTABLE_DOLLARAT;
...
sub is_exception ($) {
  ...
  if (defined $suberror) {
    if (length (my $class = blessed($e) )) {
      ...
      . '"exception substitution". DBIC tries to work around this '
  ...
  sub fail_on_internal_wantarray () {
    ...
    my ($fr, $want, $argdesc);
    {
      ...
      $argdesc = ref $DB::args[0]
        ? DBIC::_Util::refdesc($DB::args[0])
        : 'non '
      ;
    };

    if (
      $want and $fr->[0] =~ /^(?:DBIC|DBICx::)/
    ) {
      DBIC::Exception->throw( sprintf (
