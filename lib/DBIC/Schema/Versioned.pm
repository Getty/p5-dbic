...
__PACKAGE__->register_class('Table', 'DBIC::Version::Table');
...
L<DBIC::DeploymentHandler> is a much more powerful alternative to this
...
L<DBIC::Schema/ddl_filename>) then create a diff using your create DDL
...
Same as L<DBIC::Schema/deploy> but also calls C<install>.
...
normally you will have created this using L<DBIC::Schema/create_ddl_dir>.
...
sub _on_connect
{
  ...
  $self->{vschema} = DBIC::Version->connect(
    sub { $w_storage->dbh },
  ...
  unless ($self->_source_exists($vtable)) {
    my $vtable_compat = DBIC::VersionCompat->connect(sub { $w_storage->dbh })->resultset('TableCompat');
...
sub _create_db_to_schema_diff {
  ...
  unless (DBIC::Optional::Dependencies->req_ok_for ('deploy')) {
    $self->throw_exception("Unable to proceed without " . DBIC::Optional::Dependencies->req_missing_for ('deploy') );
  }
  ...
  $dbic_tr->parser('SQL::Translator::Parser::DBIC');
  $dbic_tr->data($self);
  $dbic_tr->producer($db);
