...
=head1 SYNOPSIS
 ...
 use DBIC::ResultClass::HashRefInflator;

 my $rs = $schema->resultset('CD');
 $rs->result_class('DBIC::ResultClass::HashRefInflator');
...
DBIC is faster than older ORMs like Class::DBI but it still isn't
...
$mk_hash = sub {
  ...
  my $hash = {
    ...
    map { $_ => (
      ...
      # null-branch or not
      ref $_[1]->{$_} eq $DBIC::ResultSource::RowParser::Util::null_branch_class
...
Inflates the result and prefetched data into a hash-ref (invoked by L<DBIC::ResultSet>)
...
following:
 ...
 my $cds = $artist->cds;
 $cds->result_class('DBIC::ResultClass::HashRefInflator');
 my $first = $cds->first;
...
L<DBIC::InflateColumn::DateTime>, is not performed.
