...
my $sigh = sub {
  require DBIC::_Util;
  require DBIC::SQLMaker;

  my( $meth ) = (caller(1))[3] =~ /([^:]+)$/;

  DBIC::_Util::emit_loud_diag(
    skip_frames => 1,
    msg => "The $meth() constant is now provided by DBIC::SQLMaker::ClassicExtensions: please adjust your code"
  );

  DBIC::SQLMaker::ClassicExtensions->$meth;
};
...
locked within L<DBIC::ResultSource> classes. After reimplementation
of close to 80% of the L<SQL::Abstract::Limit> functionality it was deemed
more practical to simply make an independent DBIC-specific
...
may have much better luck using L<DBIC::ResultSet/software_limit>
instead.
