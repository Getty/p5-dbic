...
a query. It is the backbone of DBIC (i.e. the really
...
by calling C<resultset> on a L<DBIC::Schema> and passing in a
L<Source|DBIC::Manual::Glossary/ResultSource> name.
...
can be used to walk through all the L<DBIC::Row>s the ResultSet
represents.
...
To add methods to your resultsets, you can subclass L<DBIC::ResultSet>, similar to:
...
See L<DBIC::Schema/load_namespaces> on how DBIC can discover and
automatically attach L<Result|DBIC::Manual::ResultClass>-specific
L<ResulSet|DBIC::ResultSet> classes.
...
|DBIC::ResultSet/new> C<< ->new($source, \%args) >>.

  use Moo;
  extends 'DBIC::ResultSet';
...
=item Arguments: L<$source|DBIC::ResultSource>, L<\%attrs?|/ATTRIBUTES>
...
L<DBIC::ResultSourceProxy::Table>) and an attribute hash (see
...
from e.g. a
C<< $schema->L<resultset|DBIC::Schema/resultset>('$source_name') >>
...
sub new {
  ...
  if (ref $class) {
    DBIC::_ENV_::ASSERT_NO_INTERNAL_INDIRECT_CALLS and fail_on_internal_call;
    return $class->new_result(@_);
  }

  my ($source, $attrs) = @_;
  $source = $source->resolve
    if $source->isa('DBIC::ResultSourceHandle');
...
=item Arguments: L<$cond|DBIC::SQLMaker> | undef, L<\%attrs?|/ATTRIBUTES>

=item Return Value: $resultset (scalar context) | L<@result_objs|DBIC::Manual::ResultClass> (list context)
...
returning a list of L<result|DBIC::Manual::ResultClass> objects instead.
...
L<Searching|DBIC::Manual::Cookbook/SEARCHING>. For a complete
documentation for the first argument, see
L<SQL::Abstract::Classic/"WHERE CLAUSES"> and its extension
L<DBIC::SQLMaker>.

For more help on using joins with search, see L<DBIC::Manual::Joining>.
...
L<DBIC::Manual::Cookbook/Formatting DateTime objects in queries>.
...
sub search {
  ...
  if (wantarray) {
    DBIC::_ENV_::ASSERT_NO_INTERNAL_WANTARRAY and my $sog = fail_on_internal_wantarray;
    return $rs->all;
  }
...
=item Arguments: L<$cond|DBIC::SQLMaker>, L<\%attrs?|/ATTRIBUTES>
...
sub search_rs {
  ...
  # legacy search
    ...
    carp_unique 'search( %condition ) is deprecated, use search( \%condition ) instead'
      unless $rsrc->result_class->isa('DBIC::CDBICompat');
...
See L<DBIC::Manual::Cookbook/SEARCHING> and
L<DBIC::Manual::FAQ/Searching> for searching techniques that do not
...
=item Return Value: L<$resultset|/search> (scalar context) | L<@result_objs|DBIC::Manual::ResultClass> (list context)
...
=item Return Value: L<$result|DBIC::Manual::ResultClass> | undef
...
order as the L<primary columns|DBIC::ResultSource/primary_columns>
...
L<unique constraint|DBIC::ResultSource/add_unique_constraint> (the
unique constraint corresponding to the
L<primary columns|DBIC::ResultSource/primary_columns> is always named
...
# DBIC::ResultSet->can('_non_unique_find_fallback') is reasonable
#
...
=item Return Value: L<$cursor|DBIC::Cursor>

=back

Returns a storage-driven cursor to the given resultset. See
L<DBIC::Cursor> for more information.
...
=item Arguments: L<$cond?|DBIC::SQLMaker>
...
=item Return Value: L<$resultsetcolumn|DBIC::ResultSetColumn>

=back

  my $max_length = $rs->get_column('length')->max;

Returns a L<DBIC::ResultSetColumn> instance for a column of the ResultSet.

=cut

sub get_column {
  my ($self, $column) = @_;
  my $new = DBIC::ResultSetColumn->new($self, $column);
  return $new;
}
...
For more information, see L<DBIC::Manual::Cookbook>.
...
sub _construct_results {
  ...
  $self->{_result_inflator}{is_core_row} = ( (
    $inflator_cref
      ==
    ( \&DBIC::Row::inflate_result || die "No ::Row::inflate_result() - can't happen" )
  ...
  $self->{_result_inflator}{is_hri} = ( (
    ! $self->{_result_inflator}{is_core_row}
      and
    $inflator_cref == (
      require DBIC::ResultClass::HashRefInflator
        &&
      DBIC::ResultClass::HashRefInflator->can('inflate_result')
    )
  ...
  if ($attrs->{_simple_passthrough_construction}) {
    ...
    if (my $check_non_null_cols = $self->{_row_parser}{$parser_type}{nullcheck} ) {
      ...
      utf8::upgrade($unrolled_non_null_cols_to_check)
        if DBIC::_ENV_::STRESSTEST_UTF8_UPGRADE_GENERATED_COLLAPSER_SOURCE;
    }
...
=item Arguments: L<$result_source?|DBIC::ResultSource>

=item Return Value: L<$result_source|DBIC::ResultSource>
...
L<"table"|DBIC::Manual::Glossary/"ResultSource"> class.
...
=item Return Value: L<$count_rs|DBIC::ResultSetColumn>

=back

Same as L</count> but returns a L<DBIC::ResultSetColumn> object.
...
=item Return Value: L<@result_objs|DBIC::Manual::ResultClass>
...
L<result component|DBIC::Manual::Component/WHAT IS A COMPONENT>.
...
This is unlike the corresponding L<DBIC::Row/update>. The user must
...
L<DBIC::Row/update>. Note that C<update_all> will run DBIC defined
...
L<in_storage|DBIC::Row/in_storage> status of any result object instances
...
L<DBIC::Row/delete>. Note that C<delete_all> will run DBIC defined
...
=item Return Value: L<\@result_objects|DBIC::Manual::ResultClass> (scalar context) | L<@result_objects|DBIC::Manual::ResultClass> (list context)
...
L<DBI/execute_for_fetch>), bypassing the L<new|DBIC::Row/new> and
L<insert|DBIC::Row/insert> calls on the
L<Result|DBIC::Manual::ResultClass> class, including any
augmentation of these methods provided by components. For example if you
are using something like L<DBIC::UUIDColumns> to create primary
...
L<Result|DBIC::Manual::ResultClass> objects or an arrayref
          ...
          if ($_->{is_rel} ||= (
            ...
            (
              ...
              ref $data->[$i][$_->{pos}] eq 'HASH'
                or
              ( defined blessed $data->[$i][$_->{pos}] and $data->[$i][$_->{pos}]->isa('DBIC::Row') )
            )
              and
            1
          )) {
            ...
            for ( ref $data->[$i][$_->{pos}] eq 'ARRAY' ? @{$data->[$i][$_->{pos}]} : $data->[$i][$_->{pos}] ) {
              if ( defined blessed $_ and $_->isa('DBIC::Row' ) ) {
        ...
        if ($colinfo->{$_}{is_rel} ||= (
          ...
          (
            ...
            ref $data->[$i]{$_} eq 'HASH'
              or
            ( defined blessed $data->[$i]{$_} and $data->[$i]{$_}->isa('DBIC::Row') )
          )
            and
          1
        )) {
...
=item Return Value: L<$pager|DBIC::ResultSet::Pager>

=back

Returns a L<DBIC::ResultSet::Pager> object tied to the current
...
L<total_entries|DBIC::ResultSet::Pager/total_entries> on the pager
object.
...
sub pager {
  ...
  require DBIC::ResultSet::Pager;
  return $self->{pager} = DBIC::ResultSet::Pager->new(
...
=item Return Value: L<$result|DBIC::Manual::ResultClass>
...
L<DBIC::Row/insert> to do that. Calling L<DBIC::Row/in_storage>
will tell you whether the result object has been inserted or not.

Passes the hashref of input on to L<DBIC::Row/new>.
...
into your storage until you call L<DBIC::Row/insert> on it.
...
(L<DBIC::Relationship/has_many>) - pass an arrayref of hashrefs.
...
bypassed more often than not. Override either L<DBIC::Row/new>
or L<DBIC::Row/insert> depending on how early in the
...
unique constraints, see L<DBIC::ResultSource/add_unique_constraint>.

If you need to know if an existing row was found or a new one created use
L</find_or_new> and L<DBIC::Row/in_storage> instead. Don't forget
to call L<DBIC::Row/insert> to save the newly created row to the
database!
...
L</update_or_new> and L<DBIC::Row/in_storage> instead. Don't forget
to call L<DBIC::Row/insert> to save the newly created row to the
database!
...
=item Return Value: L<\@result_objs|DBIC::Manual::ResultClass> | undef
...
=item Arguments: L<\@result_objs|DBIC::Manual::ResultClass>
...
term goal is to make L<DBIC> always alias the current resultset as C<me>
(and make this method unnecessary).

Thus it's currently necessary to use this method in predefined queries (see
L<DBIC::Manual::Cookbook/Predefined searches>) when referring to the
...
See L<DBIC::Schema/throw_exception> for details.
...
# Note: DBIC::ResultSet already has a _resolve-time `columns` concept,
...
L<DBIC::ResultClass::HashRefInflator>.

=cut

sub hri {
  $_[0]->search(undef, {
    result_class => 'DBIC::ResultClass::HashRefInflator'
  })
}
...
# Note: rows() already exists in DBIC::ResultSet for getting the rows
...
L<DBIC::ResultSource/resultset_attributes>.  (Please read
...
use L</select>, to instruct DBIC how to store the result of the column.
...
L<Row|DBIC::Row> object. The value will then be accessible via this
...
L<DBIC::Manual::Cookbook> for details.
...
L<Sybase ASE|DBIC::Storage::DBI::Sybase::ASE> using the
L<Generic Sub Query|DBIC::SQLMaker::LimitDialects/GenericSubQ> hack)
...
L<DBIC::ResultSetColumn> (L</get_column>). A notable exception are the
...
DBIC supports arbitrary related data prefetching from multiple related
...
L<DBIC> has no need to go back to the database when we access the
...
condition|DBIC::Relationship::Base/condition>
...
"data_type" from the L<add_columns column info|DBIC::ResultSource/add_columns>.
...
Defaults to "size" from the L<add_columns column info|DBIC::ResultSource/add_columns>,
