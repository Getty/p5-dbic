...

{
  package # Hide from PAUSE
    DBIC::Storage::NESTED_ROLLBACK_EXCEPTION;
  use base 'DBIC::Exception';
}

use DBIC::Carp;
use DBIC::Storage::BlockRunner;
use Scalar::Util qw/blessed weaken/;
use DBIC::Storage::TxnScopeGuard;
use Try::Tiny;
use namespace::clean;
...
information about L<DBI>-based storage, see L<DBIC::Storage::DBI>.
...
storage object, such as during L<DBIC::Schema/clone>.
...
Please note that all of the code in your coderef, including non-DBIC
...
It does I<not> perform an actual storage commit unless there's a DBIC
...
throw a L<DBIC::Storage::NESTED_ROLLBACK_EXCEPTION> exception,
...
sub txn_rollback {
  ...
  if ($self->transaction_depth == 1) {
    ...
    if ($self->auto_savepoint) {
      ...
      DBIC::Storage::NESTED_ROLLBACK_EXCEPTION->throw(
...
sub txn_scope_guard {
  return DBIC::Storage::TxnScopeGuard->new($_[0]);
}
...
|DBIC::Storage::Statistics/debugfh >>
...
of L<DBIC::Storage::Statistics> that is compatible with the original
...
sub debugobj {
  ...
  $self->{debugobj} ||= do {
    if (my $profile = $ENV{DBIC_TRACE_PROFILE}) {
      require DBIC::Storage::Debug::PrettyTrace;
      my @pp_args;
      ...
      # DBIC::Storage::Debug::PrettyTrace takes place
      ...
      try {
        DBIC::Storage::Debug::PrettyTrace->new(@pp_args);
      } catch {
        $self->throw_exception($_);
      }
    }
    else {
      require DBIC::Storage::Statistics;
      DBIC::Storage::Statistics->new
    }
  };
}
...
This is normally accessed via L<DBIC::Schema/connection>, which
...
If C<DBIC_TRACE_PROFILE> is set, L<DBIC::Storage::Debug::PrettyTrace>
...
L<DBIC::Storage::DBI> - reference storage implementation using
