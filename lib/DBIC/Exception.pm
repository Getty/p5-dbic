...
the default error handling of L<DBIC::Schema/throw_exception>
and derivatives.
...
This is meant for internal use by L<DBIC>'s C<throw_exception>
...
causes it to output a full trace similar to L<confess|Carp/DESCRIPTION>.
  ...
  DBIC::Exception->throw('Foo');
  try { ... } catch { DBIC::Exception->throw(shift) }
...
sub throw {
    ...
    if(!$stacktrace) {
        ...
        my ($ln, $calling) = DBIC::Carp::__find_caller(
          '^' . caller() . '$',
          'DBIC',
        );
