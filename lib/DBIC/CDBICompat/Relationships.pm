...
use Clone;
use DBIC::CDBICompat::Relationship;
use DBIC::_Util qw(quote_sub perlstring);
...
DBIC::CDBICompat::Relationships - Emulate has_a(), has_many(), might_have() and meta_info()
...
sub _declare_has_a {
  ...
  if( !$f_class->isa('DBIC::Row') and !$f_class->isa('Class::DBI::Row') ) {
...
sub _extend_meta {
    ...
    $hash{$type}{$rel} = DBIC::CDBICompat::Relationship->new($val);
