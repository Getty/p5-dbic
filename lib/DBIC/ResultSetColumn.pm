...
=head2 new
  ...
  my $obj = DBIC::ResultSetColumn->new($rs, $column);

Creates a new resultset column object from the resultset and column
passed as params. Used internally by L<DBIC::ResultSet/get_column>.
...
=item Return Value: \[ $sql, L<@bind_values|DBIC::ResultSet/DBIC BIND VALUES> ]
...
Much like L<DBIC::ResultSet/next> but just returning the
one value.
...
Much like L<DBIC::ResultSet/all> but returns values rather
than result objects.
...
Much like L<DBIC::ResultSet/reset>.
...
Much like L<DBIC::ResultSet/first> but just returning the one value.
...
Much like L<DBIC::ResultSet/single> fetches one and only one column
