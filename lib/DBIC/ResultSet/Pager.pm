...
sub entries_per_page {
  ...
  DBIC::Exception->throw( "Fewer than one entry per page!" )
    if $_[0] < 1;
...
=head1 SYNOPSIS
  ...
  my $page = DBIC::ResultSet::Pager->new();
...
entries per page. It also optionally takes the current page number:
  ...
  my $page = DBIC::ResultSet::Pager->new($total_entries, $entries_per_page, $current_page);
