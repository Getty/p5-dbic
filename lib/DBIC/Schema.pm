...
use DBIC::_Util qw(refcount quote_sub is_exception scope_guard);
...
using L<resultset_class|DBIC::ResultSource/resultset_class> to some
...
sub load_namespaces {
  ...
  my @to_register;
  {
    ...
    local *Class::C3::reinitialize = sub { } if DBIC::_ENV_::OLD_MRO;
    ...
    foreach my $source_name (@source_names_by_subclass_last) {
      ...
      if($preset_resultset_class && $preset_resultset_class ne 'DBIC::ResultSet') {
      ...
      # DBIC::Resultset and there is nothing to check
      elsif($found_resultset_class ||= $default_resultset_class) {
        $class->ensure_class_loaded($found_resultset_class);
        if(!$found_resultset_class->isa("DBIC::ResultSet")) {
            carp "load_namespaces found ResultSet class '$found_resultset_class' that does not subclass DBIC::ResultSet";
        }
  ...
  Class::C3->reinitialize if DBIC::_ENV_::OLD_MRO;
...
=item Default value: DBIC::Storage::DBI

=back

Set the storage class that will be instantiated when L</connect> is called.
If the classname starts with C<::>, the prefix C<DBIC::Storage> is
assumed by L</connect>.

You want to use this to set subclasses of L<DBIC::Storage::DBI>
...
L<DBIC::Storage::DBI::Replicated> for an example of this.
...
L<DBIC::Exception/throw>, with the exception message passed as the only
argument.
...
L<DBIC::Manual::Cookbook/Adding Indexes And Functions To Your SQL>.
...
and use L<dbh_do|DBIC::Storage::DBI/dbh_do>.
...
See L<DBIC::Storage::DBI/"connect_info"> for DBI-specific
syntax on the C<@connectinfo> argument, or L<DBIC::Storage> in
general.
...
=item Arguments: L<$source_name|DBIC::ResultSource/source_name>

=item Return Value: L<$resultset|DBIC::ResultSet>

=back

  my $rs = $schema->resultset('DVD');

Returns the L<DBIC::ResultSet> object for the registered source
name.
...
=item Return Value: L<@source_names|DBIC::ResultSource/source_name>
...
Returns the L<DBIC::ResultSource> object for the registered
source name.
...
See L<DBIC::Storage/"txn_do"> for more information.
...
L<DBIC::Storage/txn_scope_guard>.
...
L<DBIC::Storage/"txn_begin"> for more information.
...
$schema->storage->txn_commit. See L<DBIC::Storage/"txn_commit">
...
L<DBIC::Storage/"txn_rollback"> for more information.
...
Returns the L<DBIC::Storage> object for this Schema. Grab this
...
found in L<DBIC::Storage::DBI>.

=head2 populate

=over 4

=item Arguments: L<$source_name|DBIC::ResultSource/source_name>, [ \@column_list, \@row_values+ ] | [ \%col_data+ ]
...
A convenience shortcut to L<DBIC::ResultSet/populate>. Equivalent to:
...
For each L<DBIC::ResultSource> in the schema, this method creates a
...
It also attaches a corresponding L<DBIC::ResultSource> object to the
...
L<DBIC::Storage/"svp_begin"> for more information.
...
L<DBIC::Storage/"svp_release"> for more information.
...
L<DBIC::Storage/"svp_rollback"> for more information.
...
Throws an exception. Obeys the exemption rules of L<DBIC::Carp> to report
...
is nothing safe DBIC can do, aside from displaying this error. A future
...
This guard was activated beginning"
    ...
    eval {
      ...
      if( $act->(@args) ) {
        ...
        .' result in an exception. DBIC is unable to function without a reliable'
  ...
  DBIC::Exception->throw($args[0], $self->stacktrace);
}
...
Additionally, the DBIC parser accepts a C<sources> parameter as a hash
...
=item Arguments: See L<DBIC::Storage::DBI/deployment_statements>
...
=item Arguments: See L<DBIC::Storage::DBI/create_ddl_dir>
...
format.
 ...
 Prior to DBIC version 0.08100 this method had a different signature:
 ...
 in any overriding methods:
    ...
    ($dir, $version) = ($version, $dir) if ($DBIC::VERSION < 0.08100);
...
sub thaw {
  my ($self, $obj) = @_;
  local $DBIC::ResultSourceHandle::thaw_schema = $self;
...
Recommended way of dcloning L<DBIC::Row> and L<DBIC::ResultSet>
...
Registers a class which isa DBIC::ResultSourceProxy. Equivalent to
calling:
...
=item Arguments: $source_name, L<$result_source|DBIC::ResultSource>

=back

This method is called by L</register_class>.

Registers the L<DBIC::ResultSource> in the schema with the given
source name.
...
Removes the L<DBIC::ResultSource> from the schema for the given source name.
