...
DBIC::UTF8Columns - Force UTF8 (Unicode) flag on columns (DEPRECATED)
...
stored and de-serialised to unicode strings on retrieval.
  ...
  THE USE OF THIS MODULE (AND ITS COUSIN DBIC::ForceUTF8) IS VERY
...
Recently (April 2010) a bug was found deep in the core of L<DBIC>
which affects any component attempting to perform encoding/decoding by
overloading L<store_column|DBIC::Row/store_column> and
L<get_columns|DBIC::Row/get_columns>. As a result of this problem
L<create|DBIC::ResultSet/create> sends the original column values
to the database, while L<update|DBIC::ResultSet/update> sends the
encoded values. L<DBIC::UTF8Columns> and L<DBIC::ForceUTF8>
...
implications of changing such a fundamental behavior of DBIC are
...
L<join us on IRC or the mailing list|DBIC/GETTING HELP/SUPPORT>
...
Note that this module overloads L<DBIC::Row/store_column> in a way
that may prevent other components overloading the same method from working
correctly. This component must be the last one before L<DBIC::Row>
(which is provided by L<DBIC::Core>). DBIC will detect such
incorrect component order and issue an appropriate warning, advising which
components need to be loaded differently.

=head1 SEE ALSO

L<Template::Stash::ForceUTF8>, L<DBIC::UUIDColumns>.
