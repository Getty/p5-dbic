...
use DBIC::_Util qw( is_exception sigwarn_silencer );

plan skip_all => 'Test needs ' . DBIC::Optional::Dependencies->req_missing_for ('test_rdbms_pg')
  unless DBIC::Optional::Dependencies->req_ok_for ('test_rdbms_pg');
...
###  pre-connect tests (keep each test separate as to make sure rebless() runs)
  {
    ...
    SKIP: {
        skip (
          "Pg parser detection test needs " . DBIC::Optional::Dependencies->req_missing_for ('test_dt_pg'),
          2
        ) unless DBIC::Optional::Dependencies->req_ok_for ('test_dt_pg');

        my $store = ref $s->storage;
        is($store, 'DBIC::Storage::DBI', 'Started with generic storage');
      ...
      for my $t (
        ...
        $schema->txn_do( sub {
          ...
          eval {
              ...
              local *DBIC::Storage::DBI::_ping = sub { 1 }, DBIC::_ENV_::OLD_MRO && Class::C3->reinitialize()
                if DBIC::_Util::modver_gt_or_eq( 'DBD::Pg' => '3.5.0' );
