...
use DBIC::_Util qw(hrefaddr sigwarn_silencer modver_gt_or_eq modver_gt_or_eq_and_lt);
BEGIN {
...
unless (DBICTest::RunMode->is_plain) {
  ...
  $has_dt = DBIC::Optional::Dependencies->req_ok_for('test_dt_sqlite');
...

{
  ...
  my @rsets = grep {
    blessed $_
      and
    (
      $_->isa('DBIC::ResultSet')
        or
      $_->isa('DBIC::ResultSetColumn')
    )
...
for my $addr (keys %$weak_registry) {
  ...
  if ($names =~ /^Test::Builder/m) {
    ...
    # the latter is possible yet terrible: https://metacpan.org/source/RIBASUSHI/DBIC-0.082840/t/lib/DBICTest/Util/LeakTracer.pm#L113-117
    delete $weak_registry->{$addr}
