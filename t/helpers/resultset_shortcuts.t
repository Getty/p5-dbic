...
# --- order_by ---
{
  ...
  isa_ok($ordered, 'DBIC::ResultSet', 'order_by returns RS');
  my @cds = $ordered->all;
  ok(@cds > 0, 'order_by returned results');

  # Magic string - simple
  my $asc = $rs->order_by('title');
  isa_ok($asc, 'DBIC::ResultSet', 'magic order_by returns RS');

  # Magic string - DESC with !
  my $desc = $rs->order_by('!title');
  isa_ok($desc, 'DBIC::ResultSet', 'magic !order_by returns RS');

  # Magic string - comma separated
  my $multi = $rs->order_by('year,!title');
  isa_ok($multi, 'DBIC::ResultSet', 'magic comma order_by returns RS');
}

# --- distinct ---
{
  my $rs = $schema->resultset('CD')->distinct;
  isa_ok($rs, 'DBIC::ResultSet', 'distinct returns RS');
}

# --- group_by ---
{
  my $rs = $schema->resultset('CD')->group_by('year');
  isa_ok($rs, 'DBIC::ResultSet', 'group_by returns RS');
}

# --- columns ---
{
  my $rs = $schema->resultset('CD')->columns([qw/title year/]);
  isa_ok($rs, 'DBIC::ResultSet', 'columns returns RS');
  my $row = $rs->hri->first;
...
# --- add_columns ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'add_columns returns RS');
}

# --- hri ---
{
  my $rs = $schema->resultset('CD')->hri;
  isa_ok($rs, 'DBIC::ResultSet', 'hri returns RS');
  my $row = $rs->first;
...
# --- rows / limit ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'rows returns RS');
  my @all = $rs->all;
...
# --- page ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'page returns RS');
}

# --- limited_page ---
{
  my $rs = $schema->resultset('CD')->limited_page(1, 3);
  isa_ok($rs, 'DBIC::ResultSet', 'limited_page returns RS');
  my @all = $rs->all;
...
# --- prefetch ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'prefetch returns RS');
  my $cd = $rs->first;
...
# --- remove_columns ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'remove_columns returns RS');
}
...
# --- null / not_null ---
{
  ...
  isa_ok($not_null, 'DBIC::ResultSet', 'not_null returns RS');
  ok($not_null->count > 0, 'not_null finds rows');

  # With array ref
  my $multi = $schema->resultset('Artist')->not_null(['name']);
  isa_ok($multi, 'DBIC::ResultSet', 'not_null arrayref returns RS');
}

# --- like / not_like ---
{
  my $rs = $schema->resultset('CD')->like('title', '%Spoon%');
  isa_ok($rs, 'DBIC::ResultSet', 'like returns RS');

  my $not = $schema->resultset('CD')->not_like('title', '%Spoon%');
  isa_ok($not, 'DBIC::ResultSet', 'not_like returns RS');
...
# --- correlate ---
{
  ...
  isa_ok($with_cd_count, 'DBIC::ResultSet', 'correlate works in subquery');
...
# --- search_or ---
{
  ...
  isa_ok($combined, 'DBIC::ResultSet', 'search_or returns RS');
}
...
# --- union ---
{
  ...
  isa_ok($union, 'DBIC::ResultSet', 'union returns RS');
  my @all = $union->all;
...
# --- one_row ---
{
  ...
  isa_ok($cd, 'DBIC::Row', 'one_row returns Row object');
...
# --- bare ---
{
  ...
  isa_ok($bare, 'DBIC::ResultSet', 'bare returns RS');
...
# --- rand ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'rand returns RS');
  my @rows = $rs->all;
