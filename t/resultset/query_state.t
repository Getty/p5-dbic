...
# --- limit (alias for rows) ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'limit returns RS');
  my @rows = $rs->all;
...
# --- limited_page: two args ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'limited_page(page, rows) returns RS');
  my @rows = $rs->all;
...
# --- limited_page: hashref ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'limited_page(hashref) returns RS');
...
# --- limited_page: single arg (page only, must have rows already) ---
{
  ...
  isa_ok($paged, 'DBIC::ResultSet', 'limited_page(page) returns RS');
...
# --- distinct ---
{
  ...
  isa_ok($nodist, 'DBIC::ResultSet', 'distinct(0) returns RS');
}
...
# --- columns / add_columns ---
{
  ...
  isa_ok($plus, 'DBIC::ResultSet', 'add_columns returns RS');
}
...
# --- prefetch ---
{
  ...
  isa_ok($multi, 'DBIC::ResultSet', 'prefetch with multiple rels returns RS');
}
...
# --- one_row ---
{
  ...
  isa_ok($cd, 'DBIC::Row', 'one_row returns Row');
