...
sub null_branch {
  cmp_deeply(
    $_[0][0],
    $native_inflator ? undef : bless( $_[1], $DBIC::ResultSource::RowParser::Util::null_branch_class ),
  );
}
sub null_collapsed_branch {
  cmp_deeply(
    $_[0][0],
    $native_inflator ? [] : bless( $_[1], $DBIC::ResultSource::RowParser::Util::null_branch_class ),
  );
}
...

{
  ...
  sub inflate_result {
    my $class = shift;
    return map { DBIC::ResultClass::HashRefInflator->inflate_result(@_) } (1,2);
  }
}
