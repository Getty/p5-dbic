...
# --- null: single column ---
{
  ...
  isa_ok($null_rs, 'DBIC::ResultSet', 'null returns RS');
...
# --- null: multiple columns via arrayref ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'null with arrayref returns RS');
  my @rows = $rs->all;
  for my $row (@rows) {
...
# --- not_null: single column ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'not_null returns RS');
  my @rows = $rs->all;
  for my $row (@rows) {
...
# --- like: arrayref of columns ---
{
  ...
  my $rs = $schema->resultset('CD')->like(['title'], '%Spoon%');
  isa_ok($rs, 'DBIC::ResultSet', 'like with arrayref returns RS');
  my @rows = $rs->all;
  for my $row (@rows) {
...
# --- not_like: single column ---
{
  ...
  isa_ok($rs, 'DBIC::ResultSet', 'not_like returns RS');
  my @rows = $rs->all;
  for my $row (@rows) {
