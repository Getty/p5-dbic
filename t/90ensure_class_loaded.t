...
ok( $schema->ensure_class_found('DBIC::Schema'),
    'loaded package DBIC::Schema was found' );
...
# Simulate a PAR environment
{
  ...
  $retval = eval { $schema->load_optional_class('DBIC::ResultClass::HashRefInflator') };
  ok( !$@, 'load_optional_class did not throw' ) || diag $@;
  ok( $retval, 'DBIC::ResultClass::HashRefInflator loaded' );
}
