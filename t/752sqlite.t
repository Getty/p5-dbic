...
use lib qw(t/lib);
use DBICTest;
use DBIC::_Util qw( sigwarn_silencer modver_gt_or_eq modver_gt_or_eq_and_lt );
...
for my $bi ( qw(
  ...
  cmp_ok (
    ...
    (DBIC::_ENV_::IV_SIZE < 8 and $v_bits > 32) ? 'eq' : '==',
    ...
    (DBIC::_ENV_::IV_SIZE < 8 and ! modver_gt_or_eq('DBD::SQLite', '1.37')) ? $bi+0 : $bi,
...
SKIP: {
  ...
  for my $dtype (undef, \'int', \'bigint') {
    ...
    ok (
      ...
      (DBIC::_ENV_::IV_SIZE < 8 and ! modver_gt_or_eq('DBD::SQLite', '1.37')) ? $expect->bstr + 0 : $expect
