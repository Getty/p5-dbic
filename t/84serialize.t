...
my %stores = (
    ...
    dclone_func             => sub {
      local $DBIC::ResultSourceHandle::thaw_schema = $schema;
      return dclone($_[0])
    },
...
if ($ENV{DBICTEST_MEMCACHED}) {
  if (DBIC::Optional::Dependencies->req_ok_for ('test_memcached')) {
    ...
    SKIP: {
      skip 'Memcached tests need ' . DBIC::Optional::Dependencies->req_missing_for ('test_memcached'), 1;
    }
  }
}
else {
