...
plan skip_all => 'Test needs ' . DBIC::Optional::Dependencies->req_missing_for ('test_rdbms_ase')
  unless DBIC::Optional::Dependencies->req_ok_for ('test_rdbms_ase');

my @storage_types = (
  'DBI::Sybase::ASE',
  'DBI::Sybase::ASE::NoBindVars',
);
eval "require DBIC::Storage::$_;" for @storage_types;
...
my $ping_count = 0;
{
  my $ping = DBIC::Storage::DBI::Sybase::ASE->can('_ping');
  *DBIC::Storage::DBI::Sybase::ASE::_ping = sub {
    $ping_count++;
    goto $ping;
  };
}
...
for my $storage_type (@storage_types) {
  ...
  if ($storage_idx == 0 &&
      $schema->storage->isa('DBIC::Storage::DBI::Sybase::ASE::NoBindVars')) {
  ...
  isa_ok( $schema->storage, "DBIC::Storage::$storage_type" );
...
# do an IDENTITY_INSERT
  {
    ...
    my $txn_used = 0;
    my $txn_commit = \&DBIC::Storage::DBI::txn_commit;
    local *DBIC::Storage::DBI::txn_commit = sub {
      $txn_used = 1;
      goto &$txn_commit;
    };
