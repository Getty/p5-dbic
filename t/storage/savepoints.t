...
use DBIC::_Util qw(sigwarn_silencer scope_guard);
...

{
  ...
  use base 'DBIC::Storage::Statistics';
...
for ('', keys %$env2optdep) { SKIP: {
  ...
  if ($prefix = $_) {
    ...
    skip ("Testing with ${prefix}_DSN needs " . DBIC::Optional::Dependencies->req_missing_for( $env2optdep->{$prefix} ), 1)
      unless  DBIC::Optional::Dependencies->req_ok_for($env2optdep->{$prefix});

    $schema = DBICTest::Schema->connect ($dsn,$user,$pass,{ auto_savepoint => 1 });

    my $create_sql;
    $schema->storage->ensure_connected;
    if ($schema->storage->isa('DBIC::Storage::DBI::Pg')) {
    ...
    elsif ($schema->storage->isa('DBIC::Storage::DBI::mysql')) {
