...
# Test txn_scope_guard
{
  ...
  lives_ok (sub {
    ...
    warnings_exist ( sub {
      ...
      outer($s, 0);
    }, qr/A DBIC::Storage::TxnScopeGuard went out of scope without explicit commit or error. Rolling back./, 'Out of scope warning detected');
...
# make sure the guard does not eat exceptions
{
  ...
  local *{DBIC::Storage::DBI::txn_rollback} = sub { die 'die die my darling' };
...
=begin
  warnings_exist (
    ...
    [
      qr/A DBIC::Storage::TxnScopeGuard went out of scope without explicit commit or error. Rolling back./,
      qr/\*+ ROLLBACK FAILED\!\!\! \*+/,
    ],
...
# ensure Devel::StackTrace-refcapture-like effects are countered
{
  ...
  warnings_exist
    { @arg_capture = () }
    qr/\QPreventing *MULTIPLE* DESTROY() invocations on DBIC::Storage::TxnScopeGuard/
  ;
}
