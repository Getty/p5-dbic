...
sub _test_forking_action {
  ...
  for my $pid (@pids) {
    isa_ok ($schema->resultset ('Artist')->find ({ name => "forking action $pid" }), 'DBIC::Row');
  }
}
...

{
  ...
  # Test failed txn_do() with failed rollback
  {
    ...
    #local *DBIC::Storage::DBI::SQLite::txn_rollback = sub { die 'FAILED' };

    local *DBIC::Storage::DBI::txn_rollback = sub { die 'FAILED' };
...
# also test nested txn exception
{
  ...
  throws_ok( sub { $schema->txn_rollback }, 'DBIC::Storage::NESTED_ROLLBACK_EXCEPTION', 'got proper nested rollback exception' );
}
