    ...
    local $SIG{__WARN__} = sigwarn_silencer(
      qr/
        ...
        \QUnable to extract a driver name from connect info\E
          |
        \QYour storage class (DBIC::Storage::DBI) does not set sql_limit_dialect\E
      /x
    );
...
throws_ok { count_sheep($schema) } qr{I can't work out what driver to use},
    'Driver in DSN empty';
isa_ok $schema->storage, 'DBIC::Storage::DBI';
...
isa_ok $schema->storage, 'DBIC::Storage::DBI::SQLite';
...
# make sure that dynamically setting DBI_DSN post-connect works
{
  ...
  isa_ok ( $s->storage, 'DBIC::Storage::DBI::SQLite' );
}
