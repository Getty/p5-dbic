...
use DBIC::_Util 'modver_gt_or_eq_and_lt';
use base();
BEGIN {
...
# set as attr - should propagate
    ...
    is ($hri_rs->result_class, 'DBIC::ResultClass::HashRefInflator', 'result_class accessor pre-set via attribute');
    ...
    $hri_rs->result_class ('DBIC::Row'); # something bogus
    is(
        $hri_rs->search->result_class, 'DBIC::ResultClass::HashRefInflator',
...
$rs_hashrefinf->result_class('DBIC::ResultClass::HashRefInflator');
...
my $rs_artists = $schema->resultset ('Artist')->search ({ 'me.artistid' => $artist->id}, {
    prefetch => { cds => 'tracks' }, result_class => 'DBIC::ResultClass::HashRefInflator',
});
is_deeply(
  [$rs_artists->all],
