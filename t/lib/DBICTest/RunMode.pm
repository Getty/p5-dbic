...
BEGIN {
  if ($INC{'DBIC.pm'}) {
    ...
    die __PACKAGE__ . " must be loaded before DBIC (or modules using DBIC) at $frame[1] line $frame[2]\n";
  }
...
sub _check_author_makefile {
  ...
  my $optdeps = file('lib/DBIC/Optional/Dependencies.pm');
...
http://search.cpan.org/dist/DBIC/lib/DBIC.pm#GETTING_HELP/SUPPORT
...
sub is_smoker {
  return
    ( ($ENV{TRAVIS}||'') eq 'true' and ($ENV{TRAVIS_REPO_SLUG}||'') eq 'Perl5/DBIC' )
      ||
