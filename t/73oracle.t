...
plan skip_all => 'Test needs ' . DBIC::Optional::Dependencies->req_missing_for ('test_rdbms_oracle')
  unless DBIC::Optional::Dependencies->req_ok_for ('test_rdbms_oracle');
...
isa_ok (DBICTest::Schema->connect($dsn, $user, $pass)->storage->sql_maker, 'DBIC::SQLMaker::Oracle');
...
for my $use_insert_returning ($test_server_supports_insert_returning ? (1,0) : (0) ) {
  for my $force_ora_joins ($test_server_supports_only_orajoins ? (0) : (0,1) ) {
    ...
    local *DBICTest::Schema::connection = subname 'DBICTest::Schema::connection' => sub {
      ...
      $s->storage->sql_maker_class('DBIC::SQLMaker::OracleJoins') if $force_ora_joins;
      $s;
    };
...
# observed side-effect when count-is-first on a fresh env-based connect
  {
    ...
    is (ref $s2->storage, 'DBIC::Storage::DBI', 'Undetermined driver' );
