...
ok ( (! grep { $_ =~ m|DBIC| } @$inc_before ), 'Nothing DBIC related is yet loaded');

# DBIC::Optional::Dependencies queries $ENV at compile time
...
use_ok 'DBIC::Optional::Dependencies';

my $inc_after = [ keys %INC ];

is_deeply (
  [ sort @$inc_after],
  [ sort (@$inc_before, 'DBIC/Optional/Dependencies.pm') ],
  'Nothing loaded other than DBIC::OptDeps',
);

my $sqlt_dep = DBIC::Optional::Dependencies->req_list_for ('deploy');
is_deeply (
  [ keys %$sqlt_dep ],
  [ 'SQL::Translator' ],
...
# make module loading impossible, regardless of actual libpath contents
{
  ...
  ok (
    ! DBIC::Optional::Dependencies->req_ok_for ('deploy'),
    'deploy() deps missing',
  );

  like (
    DBIC::Optional::Dependencies->req_missing_for ('deploy'),
    qr/^SQL::Translator \>\= \d/,
    'expected missing string contents',
  );

  like (
    DBIC::Optional::Dependencies->req_errorlist_for ('deploy')->{'SQL::Translator'},
...
#reset cache
%DBIC::Optional::Dependencies::req_availability_cache = ();
...
is_deeply (
  DBIC::Optional::Dependencies->req_errorlist_for ('deploy'),
  {},
...
for (1..2) {
  throws_ok {
    DBIC::Optional::Dependencies->req_list_for();
