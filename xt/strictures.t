...
unless ( DBIC::Optional::Dependencies->req_ok_for ('test_strictures') ) {
  my $missing = DBIC::Optional::Dependencies->req_missing_for ('test_strictures');
  $ENV{RELEASE_TESTING}
...
my $missing_groupdeps_present = grep
  { ! DBIC::Optional::Dependencies->req_ok_for($_) }
  grep
    { $_ !~ /^ (?: test | rdbms | dist ) _ /x }
    keys %{DBIC::Optional::Dependencies->req_group_list}
;
