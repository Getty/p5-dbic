...
my $hri_fn = 'DBIC/ResultClass/HashRefInflator.pm';
...
for my $commit (`git log --format=%h HEAD ^8330454^ $hri_fn `) {
  ...
  if ($diff =~ /^ (?: \@\@ \s .+? | [+-] sub \s) \$? mk_hash /xm ) {
    ...
    push @to_bench, {
      ...
      code => scalar `git show $commit:lib/DBIC/ResultClass/HashRefInflator.pm`,
    };
...
my %bench = ( map { $_ => eval "sub {
  for (1 .. (\$num_iters{$_}||1) ) {
    DBIC::ResultClass::HashRefInflator->inflate_result(\$bench_dataset{$_})
  }
}" } qw/simple complex/ );
...
for my $bch (@to_bench) {
  Class::Unload->unload('DBIC::ResultClass::HashRefInflator');
  eval $bch->{code} or die $@;
  $INC{'DBIC/ResultClass/HashRefInflator.pm'} = $bch->{title};
